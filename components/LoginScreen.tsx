...
interface LoginScreenProps {
  onLogin: (provider: 'google' | 'facebook' | 'apple') => Promise<void>;
  lang: Language;
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, lang, setLang }) => {
  ...
  const handleLoginClick = async (provider: 'google' | 'facebook' | 'apple') => {
    setIsLoading(true);
    setError(null);
    try {
      ...
      <div className="max-w-md w-full bg-white/80 dark:bg-gray-800/60 backdrop-blur-xl p-8 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-2xl z-10 transition-colors duration-300">
        ...
        <div className="space-y-4">
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-white text-gray-800 hover:bg-gray-50 disabled:bg-gray-100 disabled:cursor-not-allowed border border-gray-300 dark:border-transparent py-3 rounded-xl font-medium transition-all transform active:scale-95 shadow-sm"
          >
            {isLoading ? (
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-[#1877F2] text-white hover:bg-[#1864D9] disabled:bg-[#1554B0] disabled:cursor-not-allowed py-3 rounded-xl font-medium transition-all transform active:scale-95 shadow-sm"
          >
            {isLoading ? (
              <svg className="w-5 h-5 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="none" opacity="0.25"></circle>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-black text-white hover:bg-gray-900 disabled:bg-gray-700 disabled:cursor-not-allowed py-3 rounded-xl font-medium transition-all transform active:scale-95 border border-gray-700 dark:border-gray-600 shadow-sm"
          >
            {isLoading ? (
              <svg className="w-5 h-5 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="none" opacity="0.25"></circle>
